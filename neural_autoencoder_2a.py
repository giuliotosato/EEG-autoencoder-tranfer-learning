...
train_data = np.load('/home/u956278/hackathon/2a/extracted_data/train.npy')
test_data = np.load('/home/u956278/hackathon/2a/extracted_data/test.npy')
train_labels = np.load('/home/u956278/hackathon/2a/extracted_data/train_labels.npy')
test_labels = np.load('/home/u956278/hackathon/2a/extracted_data/test_labels.npy')
...
def preprocessing(train_data, test_data, train_labels, test_labels):
    ...
    # create a boolean mask to select only the left and right hand labels
    mask_hand = np.logical_or(train_labels == 'left_hand', train_labels == 'right_hand')

    # create a boolean mask to remove the images and labels named both feet and tongue
    mask_remove = np.logical_or(train_labels == 'both_feet', train_labels == 'tongue')

    # combine the two masks to get the final mask to select the desired labels
    mask = np.logical_and(mask_hand, np.logical_not(mask_remove))

    # apply the mask to the features and labels arrays to get the filtered dataset
    train_data = train_data[mask]
    train_labels = train_labels[mask]

    # create a boolean mask to select only the left and right hand labels
    mask_hand = np.logical_or(test_labels == 'left_hand', test_labels == 'right_hand')

    # create a boolean mask to remove the images and labels named both feet and tongue
    mask_remove = np.logical_or(test_labels == 'both_feet', test_labels == 'tongue')
    ...
    train_labels = np.where(train_labels == 'left_hand', 0, 1)
    test_labels = np.where(test_labels == 'left_hand', 0, 1)
...
with strategy.scope():
    ...
    encoder = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(1024, 25, 1)),
        ...
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
        tf.keras.layers.Conv2D(64, [8, 5], activation='relu', padding='same', strides=[2, 5]),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
        tf.keras.layers.Conv2D(32, [4, 2], activation='relu', padding='same', strides=[2, 1]),
        tf.keras.layers.BatchNormalization(),
    ...
    decoder = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(32 * 5 * 32, activation='relu'),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
        tf.keras.layers.Reshape((32, 5, 32)),
        tf.keras.layers.Conv2DTranspose(32, [4, 2], activation='relu', padding='same', strides=[2, 1]),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
        tf.keras.layers.Conv2DTranspose(64, [8, 5], activation='relu', padding='same', strides=[2, 5]),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
...
print('original_images_shape',original_images.shape)
...
np.save('processed_data/LS_train_dataset_2a.npy', latent_space_train)

latent_space_test = encoder.predict(test_data)
np.save('processed_data/LS_test_dataset_2a.npy', latent_space_test)

reconstructed_train = autoencoder.predict(train_data)
np.save('processed_data/R_train_dataset_2a.npy', reconstructed_train)

reconstructed_test = autoencoder.predict(test_data)
np.save('processed_data/R_test_dataset_2a.npy', reconstructed_test)

np.save('processed_data/Shuffled_train_labels_2a.npy', train_labels)
np.save('processed_data/Shuffled_test_labels_2a.npy', test_labels)
np.save('processed_data/Shuffled_train_2a.npy', train_data)
np.save('processed_data/Shuffled_test_2a.npy', test_data)
