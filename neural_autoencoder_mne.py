...
train_data = np.load('/home/u956278/hackathon/MNE_data/extracted_data/train_mne.npy')
test_data = np.load('/home/u956278/hackathon/MNE_data/extracted_data/test_mne.npy')
train_labels = np.load('/home/u956278/hackathon/MNE_data/extracted_data/train_labels_mne.npy')
test_labels = np.load('/home/u956278/hackathon/MNE_data/extracted_data/test_labels_mne.npy')
...
test_labels=np.where(test_labels == -3, 1, test_labels)
...
with strategy.scope():
    ...
    encoder = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(800, 64, 1)),
        ...
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
        tf.keras.layers.Conv2D(32, [4, 2], activation='relu', padding='same', strides=[2, 2]),
        tf.keras.layers.BatchNormalization(),
    ...
    decoder = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(50 * 16 * 64, activation='relu'),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
        tf.keras.layers.Reshape((50, 16, 64)),
        tf.keras.layers.Conv2DTranspose(32, [4, 2], activation='relu', padding='same', strides=[2, 2]),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
...
np.save('processed_data/LS_train_dataset_mne.npy', latent_space_train)

latent_space_test = encoder.predict(test_data)
np.save('processed_data/LS_test_dataset_mne.npy', latent_space_test)

reconstructed_train = autoencoder.predict(train_data)
np.save('processed_data/R_train_dataset_mne.npy', reconstructed_train)

reconstructed_test = autoencoder.predict(test_data)
np.save('processed_data/R_test_dataset_mne.npy', reconstructed_test)

np.save('processed_data/Shuffled_train_labels_mne.npy', train_labels)
np.save('processed_data/Shuffled_test_labels_mne.npy', test_labels)
np.save('processed_data/Shuffled_train_mne.npy', train_data)
np.save('processed_data/Shuffled_test_mne.npy', test_data)
