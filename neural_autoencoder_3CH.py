...
train_data = np.load('/home/u956278/hackathon/3_channel/extracted_data/train_d2.npy')
test_data = np.load('/home/u956278/hackathon/3_channel/extracted_data/test_d2.npy')
train_labels = np.load('/home/u956278/hackathon/3_channel/extracted_data/train_labels_d2.npy')
test_labels = np.load('/home/u956278/hackathon/3_channel/extracted_data/test_labels_d2.npy')
...
with strategy.scope():
    ...
    encoder = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(736, 3, 1)),
        tf.keras.layers.Conv2D(256, [20, 1], activation='relu', padding='same', strides=[2, 1]),
        ...
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
        tf.keras.layers.Conv2D(64, [8, 2], activation='relu', padding='same', strides=[2, 1]),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
    ...
    decoder = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(46 * 3 * 64, activation='relu'),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
        tf.keras.layers.Reshape((46, 3, 64)),
        tf.keras.layers.Conv2DTranspose(32, [4, 2], activation='relu', padding='same', strides=[2, 1]),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
        tf.keras.layers.Conv2DTranspose(64, [8, 2], activation='relu', padding='same', strides=[2, 1]),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
        tf.keras.layers.Conv2DTranspose(128, [10, 1], activation='relu', padding='same', strides=[2, 1]),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
        tf.keras.layers.Conv2DTranspose(256, [20, 1], activation='relu', padding='same', strides=[2, 1]),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),  # added dropout layer with rate 0.2
...
np.save('processed_data/LS_train_dataset_3CH.npy', latent_space_train)

latent_space_test = encoder.predict(test_data)
np.save('processed_data/LS_test_dataset_3CH.npy', latent_space_test)

reconstructed_train = autoencoder.predict(train_data)
np.save('processed_data/R_train_dataset_3CH.npy', reconstructed_train)

reconstructed_test = autoencoder.predict(test_data)
np.save('processed_data/R_test_dataset_3CH.npy', reconstructed_test)

np.save('processed_data/Shuffled_train_labels_3CH.npy', train_labels)
np.save('processed_data/Shuffled_test_labels_3CH.npy', test_labels)
np.save('processed_data/Shuffled_train_3CH.npy', train_data)
np.save('processed_data/Shuffled_test_3CH.npy', test_data)
